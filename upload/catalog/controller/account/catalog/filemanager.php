<?php
class ControllerAccountCatalogFileManager extends Controller {
	public function index() {
		$this->load->language('account/catalog/filemanager');
    ...
    $customer_id = $this->customer->getId();
		...
		if (isset($this->request->get['directory'])) {
			$directory = rtrim(DIR_IMAGE . 'catalog/' . $customer_id . '/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
		} else {
			$directory = DIR_IMAGE . 'catalog/' . $customer_id;
		}
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href'  => $this->url->link('account/catalog/filemanager', 'token=' . $this->session->data['token'] . '&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'catalog/' . $customer_id . '/'))) . $url, 'SSL')
				);
    ...
    //$data['size'] = (preg_replace('/[^0-9]/', '',system('du -s '. DIR_IMAGE . 'catalog/' . $customer_id)) / 1048576);
		...
		$data['parent'] = $this->url->link('account/catalog/filemanager', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$data['refresh'] = $this->url->link('account/catalog/filemanager', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$pagination->limit = 16;
		$pagination->url = $this->url->link('account/catalog/filemanager', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();
    
		$this->response->setOutput($this->load->view('account/catalog/filemanager.tpl', $data));
	}
	...
	public function folder() {
		...
		/* Check user has permission
		if (!$this->user->hasPermission('modify', 'account/catalog/filemanager')) {
			$json['error'] = $this->language->get('error_permission');
		} */
    $customer_id = $this->customer->getId();
		// Make sure we have the correct directory
    ...
    $customer_id = $this->customer->getId();
		...
		foreach ($paths as $path) {
			...
			// Check path exsists
			if ($path == DIR_IMAGE . 'catalog/' . $customer_id) {
