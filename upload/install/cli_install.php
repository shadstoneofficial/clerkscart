...
//
// Command line tool for installing clerkscart
...
//                               --db_username root \
//                               --db_password pass \
//                               --db_database clerkscart \
//                               --db_driver mysqli \
//								 --db_port 3306 \
//                               --username admin \
//                               --password admin \
//                               --email youremail@example.com \
//                               --http_server http://localhost/clerkscart
//
...
define('DIR_CLERKSCART', str_replace('\\', '/', realpath(DIR_APPLICATION . '../')) . '/');
...
function check_requirements() {
	$error = null;
	if (phpversion() < '5.0') {
		$error = 'Warning: You need to use PHP5 or above for clerkscart to work!';
	}
	...
	if (ini_get('session.auto_start')) {
		$error = 'Warning: clerkscart will not work with session.auto_start enabled!';
	}

	if (!extension_loaded('mysqli')) {
		$error = 'Warning: MySQLi extension needs to be loaded for clerkscart to work!';
	}

	if (!extension_loaded('gd')) {
		$error = 'Warning: GD extension needs to be loaded for clerkscart to work!';
	}

	if (!extension_loaded('curl')) {
		$error = 'Warning: CURL extension needs to be loaded for clerkscart to work!';
	}

	if (!function_exists('mcrypt_encrypt')) {
		$error = 'Warning: mCrypt extension needs to be loaded for clerkscart to work!';
	}

	if (!extension_loaded('zlib')) {
		$error = 'Warning: ZLIB extension needs to be loaded for clerkscart to work!';
	}
...
function setup_db($data) {
	...
	$file = DIR_APPLICATION . 'clerkscart.sql';
	...
	if ($lines) {
		...
		$db->query("INSERT INTO `" . $data['db_prefix'] . "setting` SET `code` = 'config', `key` = 'config_url', value = '" . $db->escape(HTTP_CLERKSCART) . "'");
...
function write_config_files($options) {
  ...
  $output .= 'define(\'HTTPS_CATALOG\', \'' . $options['http_server'] . '\');' . "\n";
	...
	$output .= '// DIR' . "\n";
	$output .= 'define(\'DIR_APPLICATION\', \'' . DIR_CLERKSCART . 'catalog/\');' . "\n";
	$output .= 'define(\'DIR_SYSTEM\', \'' . DIR_CLERKSCART . 'system/\');' . "\n";
	$output .= 'define(\'DIR_DATABASE\', \'' . DIR_CLERKSCART . 'system/database/\');' . "\n";
	$output .= 'define(\'DIR_LANGUAGE\', \'' . DIR_CLERKSCART . 'catalog/language/\');' . "\n";
	$output .= 'define(\'DIR_TEMPLATE\', \'' . DIR_CLERKSCART . 'catalog/view/theme/\');' . "\n";
	$output .= 'define(\'DIR_CONFIG\', \'' . DIR_CLERKSCART . 'system/config/\');' . "\n";
	$output .= 'define(\'DIR_IMAGE\', \'' . DIR_CLERKSCART . 'image/\');' . "\n";
	$output .= 'define(\'DIR_CACHE\', \'' . DIR_CLERKSCART . 'system/storage/cache/\');' . "\n";
	$output .= 'define(\'DIR_DOWNLOAD\', \'' . DIR_CLERKSCART . 'system/storage/download/\');' . "\n";
	$output .= 'define(\'DIR_UPLOAD\', \'' . DIR_CLERKSCART . 'system/storage/upload/\');' . "\n";
	$output .= 'define(\'DIR_MODIFICATION\', \'' . DIR_CLERKSCART . 'system/storage/modification/\');' . "\n";
	$output .= 'define(\'DIR_LOGS\', \'' . DIR_CLERKSCART . 'system/storage/logs/\');' . "\n\n";
	...
	$file = fopen(DIR_CLERKSCART . 'config.php', 'w');
	...
	$output .= '// DIR' . "\n";
	$output .= 'define(\'DIR_APPLICATION\', \'' . DIR_CLERKSCART . 'admin/\');' . "\n";
	$output .= 'define(\'DIR_SYSTEM\', \'' . DIR_CLERKSCART . 'system/\');' . "\n";
	$output .= 'define(\'DIR_DATABASE\', \'' . DIR_CLERKSCART . 'system/database/\');' . "\n";
	$output .= 'define(\'DIR_LANGUAGE\', \'' . DIR_CLERKSCART . 'admin/language/\');' . "\n";
	$output .= 'define(\'DIR_TEMPLATE\', \'' . DIR_CLERKSCART . 'admin/view/template/\');' . "\n";
	$output .= 'define(\'DIR_CONFIG\', \'' . DIR_CLERKSCART . 'system/config/\');' . "\n";
	$output .= 'define(\'DIR_IMAGE\', \'' . DIR_CLERKSCART . 'image/\');' . "\n";
	$output .= 'define(\'DIR_CACHE\', \'' . DIR_CLERKSCART . 'system/storage/cache/\');' . "\n";
	$output .= 'define(\'DIR_DOWNLOAD\', \'' . DIR_CLERKSCART . 'system/storage/download/\');' . "\n";
	...
	$output .= 'define(\'DIR_LOGS\', \'' . DIR_CLERKSCART . 'system/storage/logs/\');' . "\n";
	$output .= 'define(\'DIR_MODIFICATION\', \'' . DIR_CLERKSCART . 'system/storage/modification/\');' . "\n";
	$output .= 'define(\'DIR_CATALOG\', \'' . DIR_CLERKSCART . 'catalog/\');' . "\n\n";
	...
	$file = fopen(DIR_CLERKSCART . 'admin/config.php', 'w');
...
function dir_permissions() {
	$dirs = array(
		DIR_CLERKSCART . 'image/',
		DIR_CLERKSCART . 'system/storage/download/',
		DIR_CLERKSCART . 'system/storage/upload/',
		DIR_CLERKSCART . 'system/storage/cache/',
		DIR_CLERKSCART . 'system/storage/logs/',
		DIR_CLERKSCART . 'system/storage/modification/',
	);
...
case "install":
	try {
		...
		define('HTTP_CLERKSCART', $options['http_server']);
		$valid = valid($options);
		if (!$valid[0]) {
